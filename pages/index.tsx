...
import { info, error as logError, warning } from '../src/utils/browserLogging'
...
const Home: NextPageWithLayout = () => {
  ...
  useEffect(() => {
    if (isNil(session?.status)) {
      warning({ message: 'Hosted Session doesnt have a status', session })
    ...
    if (typeof window === 'undefined') {
      logError({ session }, 'Window is not defined')
      addSentryBreadcrumb({
        category: BreadcrumbCategory.GENERIC,
    ...
    switch (session?.status) {
      case HostedSessionStatus.Completed:
        info({ message: 'Hosted Session completed', session })
        addSentryBreadcrumb({
      ...
      case HostedSessionStatus.Expired:
        info({ message: 'Hosted Session Expired', session })
        addSentryBreadcrumb({
